    ...
    <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-100 px-6 py-4 shadow-sm">
      <div className="max-w-7xl mx-auto flex justify-between items-center gap-4">
        ...
        <div className="flex gap-2 bg-slate-100 p-1.5 rounded-full border border-slate-200">
          ...
          <Link href="/fr" className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-bold transition-all ${lang === 'fr' ? 'bg-white shadow-md text-purple-700' : 'text-slate-500 hover:text-slate-800'}`}>
            <span className="text-lg">🇫🇷</span> FR
          </Link>
          <Link href="/en" className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-bold transition-all ${lang === 'en' ? 'bg-white shadow-md text-purple-700' : 'text-slate-500 hover:text-slate-800'}`}>
